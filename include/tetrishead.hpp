...
const std::vector<chtype> BLOCKS{
    ' ' | COLOR_PAIR(BRICK_R_CELL), ' ' | COLOR_PAIR(BRICK_G_CELL),
    ' ' | COLOR_PAIR(BRICK_Y_CELL), ' ' | COLOR_PAIR(BRICK_B_CELL),
    ' ' | COLOR_PAIR(BRICK_C_CELL), ' ' | COLOR_PAIR(BRICK_M_CELL),
};
...
const Koords SHOW_POSITION{
    Koords{END_SCR + Koords(-END_SCR.getY() + BEG_SCR.getY() + 3, 10)}};
